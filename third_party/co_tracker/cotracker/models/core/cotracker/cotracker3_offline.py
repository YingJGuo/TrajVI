...
class CoTrackerThreeOffline(CoTrackerThreeBase):
    ...
    def encode_video_features(self, video, fmaps_chunk_size=200):
        """Encode reusable frame features for query-dependent tracking calls."""
        video = 2 * (video / 255.0) - 1.0
        B, T, C, H, W = video.shape
        dtype = video.dtype
        if T > fmaps_chunk_size:
            chunks = []
            for start in range(0, T, fmaps_chunk_size):
                video_chunk = video[:, start:start + fmaps_chunk_size]
                encoded = self.fnet(video_chunk.reshape(-1, C, H, W))
                chunk_t = video_chunk.shape[1]
                chunk_c, chunk_h, chunk_w = encoded.shape[1:]
                chunks.append(encoded.reshape(
                    B, chunk_t, chunk_c, chunk_h, chunk_w))
            fmaps = torch.cat(chunks, dim=1).reshape(
                -1, chunk_c, chunk_h, chunk_w)
        else:
            fmaps = self.fnet(video.reshape(-1, C, H, W))
        fmaps = fmaps.permute(0, 2, 3, 1)
        fmaps = fmaps / torch.sqrt(torch.maximum(
        ...
        for _ in range(self.corr_levels - 1):
            pooled = F.avg_pool2d(
                fmaps.reshape(B * T, self.latent_dim,
                              fmaps.shape[-2], fmaps.shape[-1]),
                2, stride=2)
            fmaps = pooled.reshape(
                B, T, self.latent_dim, pooled.shape[-2], pooled.shape[-1])
    ...
    def forward(
        ...
        if fmaps_pyramid is None:
            fmaps_pyramid = self.encode_video_features(
                (video + 1.0) * 127.5,
