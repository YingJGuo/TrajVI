...
class CoTrackerPredictor(torch.nn.Module):
    ...
    def forward(
        ...
        if queries is None and grid_size == 0:
            outputs = self._compute_dense_tracks(
                video,
            ...
            outputs = self._compute_sparse_tracks(
                video,
                queries,
                segm_mask,
                grid_size,
    ...
    def _compute_dense_tracks(
        ...
        *_, H, W = video.shape
        ...
        tracks = visibilities = raw_visibilities = confidences = None
        ...
        for offset in range(grid_step * grid_step):
            ...
            tracks_step, visibilities_step = outputs[:2]
            tracks = smart_cat(tracks, tracks_step, dim=2)
            visibilities = smart_cat(visibilities, visibilities_step, dim=2)
            if return_scores:
                raw_visibilities = smart_cat(raw_visibilities, outputs[2], dim=2)
                confidences = smart_cat(confidences, outputs[3], dim=2)

        if return_scores:
            return tracks, visibilities, raw_visibilities, confidences
    ...
    def _compute_sparse_tracks(
        ...
        tracks, visibilities, confidence, *_ = self.model.forward(
            video=video,
            queries=queries,
            iters=self.iterations if iters is None else int(iters),
        )

        if backward_tracking:
            tracks, visibilities, confidence = self._compute_backward_tracks(
                video, queries, tracks, visibilities, confidence, iters=iters
            )
            if add_support_grid:
        ...
        raw_visibilities = visibilities
        thr = 0.9
        visibilities = raw_visibilities > thr
        ...
        if return_scores:
            return tracks, visibilities, raw_visibilities, confidence
        return tracks, visibilities

    def _compute_backward_tracks(
        self, video, queries, tracks, visibilities, confidence, iters=None
    ):
        inv_video = video.flip(1).clone()
        inv_queries = queries.clone()
        inv_queries[:, :, 0] = inv_video.shape[1] - inv_queries[:, :, 0] - 1

        inv_tracks, inv_visibilities, inv_confidence, *_ = self.model(
            video=inv_video,
            queries=inv_queries,
            iters=self.iterations if iters is None else int(iters),
        )
        ...
        inv_confidence = inv_confidence.flip(1)
        ...
        confidence[mask[:, :, :, 0]] = inv_confidence[mask[:, :, :, 0]]
